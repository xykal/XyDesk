...
export function CustomControlMapping({send,onToggleMode}:{send:(b:Uint8Array)=>void;onToggleMode?:()=>void}){
 ...
 {selectedItem.kind==='key'?<MappingPicker label="Tombol" options={KEY_OPTIONS} values={[selectedItem.code]} onChange={([code])=>update({code:Number(code),label:String(KEY_OPTIONS.find(x=>x[0]===code)?.[1]||code)})}/>:selectedItem.kind==='chord'?<MappingPicker label="Kombinasi" multiple options={KEY_OPTIONS} values={selectedItem.keys||[]} onChange={keys=>update({keys:chordKeys(keys)})}/>:selectedItem.kind==='toggle'?<p>Switch mode: mengetuk tombol ini berganti antara gerak langsung dan trackpad.</p>:<MappingPicker label="Tombol mouse / scroll" options={selectedItem.kind==='mouse'?[[0,'Kiri'],[1,'Kanan'],[2,'Tengah'],[3,'Samping kembali'],[4,'Samping maju']]:selectedItem.kind==='scrollX'?[[120,'Ke kanan'],[-120,'Ke kiri']]:[[120,'Ke atas'],[-120,'Ke bawah']]} values={[selectedItem.code]} onChange={([code])=>update({code:Number(code)})}/>}
