...
export function SessionHistoryPage({renderReconnect}:{renderReconnect?:(item:HistoryItem)=>ReactNode}={}){
 ...
 {error&&<p role="alert">{error}</p>}{loading?<p role="status">Memuat riwayat…</p>:!items.length&&!error?<div className="history-empty"><img className="history-empty-art" src="/illustrations/empty-history.webp" alt="" width="640" height="640" loading="lazy" decoding="async"/><h2>Belum ada sesi tersimpan</h2><p>Setelah sesi selesai atau terputus, detail PC muncul di sini. Wallpaper HD diambil otomatis tanpa menangkap aplikasi terbuka dan dipakai ulang untuk ID yang sama.</p></div>:null}
