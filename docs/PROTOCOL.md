...
- Biner `0x0C mode:u8` (tepat 2 byte): 0=720p, 1=1080p, 2=native maksimal 4096×2160. Ini batas gambar kirim, bukan perubahan resolusi/scaling Windows. Mode 720p adalah kontrak HD mutlak: encoder mengirim canvas tepat 1280×720, termasuk saat capture RDP lebih kecil atau rasio sumber sedikit berbeda; seluruh desktop dipetakan tanpa crop atau stretch. Jika rasio sumber bukan 16:9, encoder memakai letterbox internal dan mengirim `contentRect` agar input mengabaikan area bar. Mode 1080p/native tetap mengikuti batas SDP constrained-baseline packetization-mode=1: Level4.0 → maksimal1920×1080; Level5.1 → native hingga4096×2160. Mode native Level5.1 dibatasi15fps; lainnya30fps. Software maksimum14Mbps. Ukuran frame yang diterima decoder tetap sumber kebenaran client dan ditampilkan sebagai `teramati` di HUD.
