...
- **Panel Windows dirombak ulang dari umpan balik lapangan pemilik (2026-09-25):** setelah dites sungguhan di Windows, panel lama (560px, satu kolom, bayangan luar) dinilai kurang: "ga lebar, gada sidebar, gaperlu ada shadow". Sekarang panel 960×600 dengan sidebar navigasi tiga bagian — **Status** (kartu status + Device ID + kode pairing), **Kontrol** (Mulai/Hentikan/Restart + tautan web/log), **Bantuan** — dan bayangan luar dihapus total; tepi hanya dihaluskan satu-dua piksel. Kontrol baku tidak lagi tombol teks semua: mulai/hentikan/restart/salin/web/log/navigasi memakai ikon vektor yang digambar sendiri (segitiga putar, kotak, busur panah, bola, lembar, dua lembar), teks tinggal untuk nilai milik pengguna dan judul bagian. Hit-test, urutan Tab, dan probe CI mengerti bagian aktif: kontrol yang tidak tampil tidak bisa tertekan. Gerbang CI diperbarui (ukuran jendela 964×604, margin tepi 2, daftar hit-test baru) dan pemeriksa bentuk kini membuktikan ketiadaan bayangan. Uji tata letak Linux 108 pemeriksaan.
- **Keyboard virtual web tidak lagi huruf besar semua (2026-09-25):** keycap huruf di keyboard virtual sesi kini mengikuti mode huruf sungguhan — kecil saat Caps/Shift mati, besar saat hidup — sehingga tidak terlihat "berteriak" dan pengguna yakin tombolnya benar.
- **Layar hitam di RDP kini punya diagnosis, bukan misteri (2026-09-25):** uji lapangan menampilkan sesi tersambung tapi layar hitam walau capture melaporkan frame jalan. Dua telemetri baru di host: (1) detektor isi hitam — capture GDI yang ±2 detik terus membaca piksel hitam menandai `blackSuspected` dan menulis peringatan sekali; (2) pemeriksa sesi — GDI hanya melihat desktop sesinya sendiri, jadi bila proses host berada di sesi berbeda dari sesi konsol aktif (klasik: service di sesi 0, pengguna di sesi RDP), host memperingatkan saat capture mulai dan melaporkan `sessionMismatch` di `/status`. Peringatannya menyebut tindakan konkretnya: jalankan ulang host dari dalam sesi yang dipakai. Batas jujur: perbaikan ini membuat penyebab terbaca, bukan menyembuhkan isolasi sesi Windows — itu memang sifat GDI.
