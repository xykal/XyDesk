...
- **Ilustrasi status dan ikon HUD mouse di web (2026-09-23):** overlay sesi (`session-connecting`) kini memakai ilustrasi sesuai fase — monitor dengan jam pasir selagi pairing/negotiating, dan kabel terlepas dengan segitiga peringatan untuk gagal/ditolak/sesi berakhir — menggantikan logo statis di overlay itu. Riwayat kosong mendapat gambar arsip, dan panel "Cara main" di Connect dapat gambar di samping langkah (client: gestur sentuh; host: pairing). Aset webp transparan 640px ada di `web/public/illustrations/` (~494 KB total). Tombol kiri/kanan HUD mouse digambar ulang gaya gaming (bodi sculpt, tombol aktif terisi, tombol samping, garis lampu bawah aksen lilac), dan dua tombol yang sebelumnya karakter teks (`⇄`, `⌖`) jadi ikon SVG trackpad dan kursor. Tanpa bump versi. Overlay sesi diverifikasi lewat build dan pratinjau halaman connect/riwayat/landing — belum diuji berhadapan dengan host nyata.
