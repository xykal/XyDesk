...
- **Installer MSI bisa dibangun lagi (2026-09-23):** job lint installer di `main` merah dengan `WIX0144: The extension 'WixToolset.UI.wixext' could not be found` (run `35879908181`) — WiX v4 terpasang, tetapi extension UI yang dipakai `wix build -ext` tidak pernah dimasukkan ke cache global, dan tidak ada satu pun `wix extension add` di seluruh workflow. Kini `build.yml`, `release.yml`, dan `prepare-windows-installer.yml` memasang `WixToolset.UI.wixext/4.0.6` (versi disamakan tool WiX-nya) sebelum membangun MSI, dengan `throw` bila gagal. Jalur MSI yang dikirim bersama NSIS sejak perubahan installer dual-format kini benar-benar bisa lulus.
